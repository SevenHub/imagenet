...
saveH5FilePath = savePath+'lstm15day10up.h5'
...
class Config:
    ...
    feature_back_days = 15 #每个input的长度 使用过去10天的数据
...
#         test_input_aux = []
        ...
        for i in range(0, len(traindata)-conf.feature_back_days-conf.feature_days):
            ...
            c = 0        
            if i+conf.feature_back_days<len(data['open']):
                if maxClose/data['open'][i+conf.feature_back_days] - 1 > 0.1:
                    c = 1
#             c =  100*(maxClose/data['open'][i+conf.feature_back_days] - 1)   
#                 print(data['date'][i])
...
#         return_sequences=True,
        model.add(LSTM(64, input_shape=(conf.feature_back_days,6)))  # returns a sequence of vectors of dimension 32
