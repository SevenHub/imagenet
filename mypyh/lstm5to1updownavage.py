...
from tensorflow.keras.layers import Input,LSTM,Dense,Dropout,Conv1D,Conv3D, MaxPooling1D,Activation,Bidirectional,MaxPooling2D,ConvLSTM2D,BatchNormalization,ZeroPadding1D,Flatten,GlobalAveragePooling1D
...
saveH5FilePath = savePath+'lstm5to1updownavage.h5'

pd.set_option('display.max_columns',100)
#常量
class Config:
    source_fields = ['date','open','high','low','close','volume','amount']  # features
    fields = ['open','high','low','close','avage']
...
#                            input_shape=(conf.feature_back_days,5,1),
...
#         model.add(LSTM(128,return_sequences=True, input_shape=(conf.feature_back_days,6)))  # returns a sequence of vectors of dimension 32
        model.add(LSTM(32,return_sequences=True, input_shape=(conf.feature_back_days,5)))    
           
        #连接下面的 所以 return_sequences=True
        model.add(Bidirectional(LSTM(32,return_sequences=True)))#合并来自两个并行LSTMs的输出
         
        model.add(LSTM(32))  # 返回维度为 32 的单个向量 ,return_sequences=True
...
#         model.add(Dense(32, activation='relu'))
        ...
        model.add(BatchNormalization())#加速训练过程
